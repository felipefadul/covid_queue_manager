...
async function handleQueueCall (data, nome_medico, codigo_classificacao, nome_fila)
{
  ...
  await db.inserirTabelaHistorico(paciente_id, nome_enfermeiro, nome_medico, tipo_classificacao_id[0].tipo_classificacao_id);
...
class GerenciadorFila {
  ...
  async chamarPaciente(req, res) {
    ...
    try {
      ...
      if (data != -1)
      {
        var paciente_id = await handleQueueCall (data, nome_medico, CODIGO_RISCO_ALTO, 'grave');
      }
      else {
        ...
        if (((contagem % 9 === 0) && (data_sem_sintomas != -1)) || ((data_leve === -1) && (data_moderado === -1)) ) {
          paciente_id = await handleQueueCall (data_sem_sintomas, nome_medico, CODIGO_SEM_SINTOMAS, 'sem_sintomas');
        }
        else if (((contagem % 3 === 0) && (data_leve != -1)) || data_moderado === -1) {
          paciente_id = await handleQueueCall (data_leve, nome_medico, CODIGO_RISCO_LEVE, 'leve');
        }
        else if (data_moderado != -1) {
          paciente_id = await handleQueueCall (data_moderado, nome_medico, CODIGO_RISCO_MODERADO, 'moderado');
        }
      }
